...
function getShares(blocks, callback) {
    ...
    redisClient.multi(redisCommands).exec(function(error, replies) {
        ...
        for (var i = 0; i < replies.length; i++) {
            ...
            var devFee = config.fees.devFee ? config.fees.devFee : 0;
...
function handleOrphan(blocks, callback) {
    ...
    blocks.forEach(function(block) {
        ...
        orphanCommands.push(['del', redisPrefix + ':solver:round' + block.height]);
...
function handleUnlocked(blocks, callback) {
    ...
    blocks.forEach(function(block) {
		...
		var feePercent = config.fees.poolFee / 100;
        var solverPercent = (config.blockUnlocker.solverReward || 0) / 100;
        var solver = "";

        var reward = Math.round(block.reward - (block.reward * (feePercent + solverPercent)));
        var solverReward = Math.round(block.reward * solverPercent);

        redisClient.get(redisPrefix + ':solver:round' + block.height, function(error, result){
            if (error){
        ...
        log('info', logSystem, 'Unlocked %d block with reward %d. Solver reward: %d', [block.height, (block.reward/10000).toFixed(4), feePercent, reward, solverReward]);
